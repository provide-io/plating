...
1. **Attrs for Data Classes**: Uses `@attrs.define` instead of dataclasses
...
1. **Async-Only Design**: All rendering and I/O operations are async
...
1. Register in `TemplateEngine` class
1. Update `async_renderer.py` to include in env.globals
...
1. Tightly coupled to pyvider ecosystem for component discovery
1. No plugin system for alternative schema extractors
