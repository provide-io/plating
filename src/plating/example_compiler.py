...
class ExampleCompiler:
    ...
    def compile_examples(
        ...
        for bundle in bundles:
            if component_types and bundle.component_type not in component_types:
                continue

            if bundle.component_type not in bundles_by_type:
                bundles_by_type[bundle.component_type] = []
            bundles_by_type[bundle.component_type].append(bundle)
    ...
    def _compile_component_examples(
        ...
        for example_name, example_content in examples.items():
            self._generate_individual_example(
                bundle, component_dir, example_name, example_content, result
            )
...
provider "{self.provider_name}" {{
  ...
  # Add your configuration options here
    ...
    def _build_showcase_content(self, bundles: list[PlatingBundle]) -> str:
        ...
        for bundle in bundles:
            if bundle.component_type not in by_type:
                by_type[bundle.component_type] = []
            by_type[bundle.component_type].append(bundle)
...
'''
    ...
    def _generate_example_readme(self, bundle: PlatingBundle, example_name: str, content: str) -> str:
        ...
        return f"""# {bundle.component_type.value.replace('_', ' ').title()}: {bundle.name} - {example_name} Example

This directory contains a complete, executable Terraform example demonstrating the `{bundle.name}` {bundle.component_type.value.replace('_', ' ')}.
...
"""
    ...
    def _generate_combined_readme(self, bundle: PlatingBundle, examples: dict[str, str]) -> str:
        ...
        example_list = "\n".join(f"- **{name}**: {self._extract_first_line(content)}"
                                 for name, content in examples.items())

        return f"""# {bundle.component_type.value.replace('_', ' ').title()}: {bundle.name} - Complete Examples

This directory contains a comprehensive Terraform example demonstrating all capabilities of the `{bundle.name}` {bundle.component_type.value.replace('_', ' ')}.
...
"""
    ...
    def _generate_showcase_readme(self, bundles: list[PlatingBundle]) -> str:
        ...
        component_list = "\n".join(f"- {bundle.component_type.value.replace('_', ' ').title()}: `{bundle.name}`"
                                   for bundle in bundles)
