...
def generate_docs(
    ...
    Args:
        ...
        package_name: Package to search for garnish bundles
        component_type: Optional filter for component type
        force: Force overwrite existing files
    """
    # Discover bundles
    discovery = GarnishDiscovery(package_name)
    bundles = discovery.discover_bundles(component_type)

    if not bundles:
        logger.warning(f"No garnish bundles found in {package_name}")
        return

    logger.info(f"Found {len(bundles)} garnish bundles")
