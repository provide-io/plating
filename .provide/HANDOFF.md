...
- Group name `"pyvider"` ≠ module name `"pyvider.components"`
...
1. ✅ Component discovery returns >0 components for both packages
2. ✅ Schema extraction produces non-empty schema dictionaries
3. ✅ `{{ schema() }}` template function renders populated tables
4. ✅ All "Argument Reference" sections have content
5. ✅ Documentation builds successfully for both packages
6. ✅ Selective discovery works (one package at a time)
7. ✅ No "None" categories in documentation
...
1. **Replaced Foundation Hub with Pyvider's ComponentDiscovery** ✅
2. **Implemented Module-Based Filtering for Selective Discovery** ✅  
3. **Fixed PlatingBundle Template Detection** ✅ (Partial - see below)

### Part 1: ComponentDiscovery Integration (COMPLETED ✅)
...
- **Updated exports**: Renamed `get_component_schemas_from_hub` → `get_component_schemas_from_registry`
...
### Part 2: Module-Based Filtering (COMPLETED ✅)
...
- **Added `_get_module_prefix_from_package_name()` method** with mappings:
  - `"pyvider-components"` → `"pyvider.components"`
  - `"tofusoup"` / `"terraform-provider-tofusoup"` → `"tofusoup"`
  - `"tofusoup.tf.components"` → `"tofusoup"`
...
### Part 3: PlatingBundle Template Detection Fix (COMPLETED ✅)
...
- Looked for: `"provider_info.tmpl.md"` ❌  
- Actual file: `"tofusoup_provider_info.tmpl.md"` ✅
...
### Results: pyvider-components (FULLY WORKING ✅)

**Documentation Build:**
```
✅ Generated 41 files in 0.2s
```
...
✅ **pyvider-components documentation is FULLY POPULATED with schemas!**

### Results: tofusoup (PARTIALLY WORKING ⚠️)

**Template Detection:** ✅ FIXED - 14 data sources detected with templates
**Schema Extraction:** ✅ WORKING - 9 tofusoup data source schemas extracted
**Documentation Build:** ⚠️ ISSUE - "Generated 0 files" 
...
1. `registry.get_components_with_templates(ComponentType.DATA_SOURCE)` returns 14 bundles ✅
2. Schema extraction with module_prefix="tofusoup" returns 9 data sources ✅
3. Templates detected correctly via has_main_template() ✅
4. But doc_generator.py still generates 0 files ❌
...
- ✅ ComponentDiscovery discovers 6 resources, 17 data sources, 26 functions total
- ✅ Module filtering with "pyvider.components" returns only pyvider components
- ✅ Module filtering with "tofusoup" returns only tofusoup components

### Template Detection Tests
- ✅ has_main_template() returns True for tofusoup bundles
- ✅ get_components_with_templates() returns 14 data sources (10 from tofusoup)

### Documentation Build Tests
- ✅ pyvider-components: 41 files generated, all schemas populated
- ⚠️ tofusoup: Templates detected but 0 files generated (requires further investigation)

### Regression Tests
- ✅ pyvider-components documentation still builds correctly
- ✅ No breaking changes to existing functionality
- ✅ Async patterns work correctly throughout
...
|----------|--------|-------|
| Component discovery returns >0 components | ✅ PASS | 6 resources, 17 data sources, 26 functions |
| Schema extraction produces non-empty dicts | ✅ PASS | Both pyvider and tofusoup schemas extracted |
| `{{ schema() }}` renders populated tables | ✅ PASS | Verified in pyvider-components docs |
| All "Argument Reference" sections have content | ⚠️ PARTIAL | Works for pyvider-components, not tofusoup |
| Documentation builds successfully | ⚠️ PARTIAL | pyvider-components ✅, tofusoup ⚠️ |
| Selective discovery works | ✅ PASS | Module filtering successfully isolates packages |
| No "None" categories | ✅ PASS | Categories display correctly |

## Conclusion

The core issue of missing schemas in plating documentation has been **successfully resolved** for pyvider-components through:

1. Replacing Foundation Hub with Pyvider's ComponentDiscovery ✅
2. Implementing module-based filtering for selective discovery ✅
3. Fixing PlatingBundle template detection ✅
...
3. Registry tries to resolve distribution name → module name via `_distribution_to_module_name()`
...
- pyvider-components: distribution name `"pyvider-components"` starts with top-level `"pyvider"` → heuristic works
- tofusoup: distribution name `"terraform-provider-tofusoup"` doesn't start with `"tofusoup"` → falls back to broken entry point lookup
...
```
🔍 Filtering to package: tofusoup.tf.components
🔹 Using component package from [pyvider] section: tofusoup.tf.components
```

**Documentation generation:**
```
✅ Generated 9 files in 0.1s
  • docs/data-sources/provider_info.md
  • docs/data-sources/provider_versions.md
  • docs/data-sources/registry_search.md
  • docs/data-sources/module_search.md
  • docs/data-sources/state_resources.md
  • docs/data-sources/module_info.md
  • docs/data-sources/module_versions.md
...
✅ **Schemas are now fully populated in tofusoup documentation!**
...
| Component discovery returns >0 components | ✅ PASS | 6 resources, 17 data sources, 26 functions |
| Schema extraction produces non-empty dicts | ✅ PASS | Both pyvider and tofusoup schemas extracted |
| `{{ schema() }}` renders populated tables | ✅ PASS | Verified in both pyvider-components and tofusoup docs |
| All "Argument Reference" sections have content | ✅ PASS | Works for both pyvider-components AND tofusoup |
| Documentation builds successfully | ✅ PASS | pyvider-components ✅, tofusoup ✅ |
| Selective discovery works | ✅ PASS | Module filtering successfully isolates packages |
| No "None" categories | ✅ PASS | Categories display correctly |
...
1. ✅ ComponentDiscovery integration - Components discovered correctly
2. ✅ Module-based filtering - Selective package documentation works
3. ✅ PlatingBundle template detection - Provider-prefixed templates detected
4. ✅ Auto-detection configuration - Explicit `[pyvider] component_packages` bypasses heuristic issues
